...
def create_product(product_data, variant_data, option_data, r, odoo):
    ...
    if product_id:
        ...
        # product_id = odoo.search('product.template', [('id', '=', product_id)], lang=lang)
        #return product_id
    else:
        ...
        # )
        product_id = odoo.search('product.template', [('default_code', '=', product_default_code)], lang=lang)

    print(product_id)
    #exit()

    curency = os.getenv(store_mark + '_CURRENCY')

    currency_id = odoo.search('res.currency', [('name', '=', curency)], lang=lang)
    ...
    if product_id:
        ...
        #     }]
        # )
        product_id = odoo.create('product.template', {
            'name': product_data['name'],
            'description': product_data['description'],
            'list_price': product_data['price'],
            'compare_list_price': compare_at_price,
    ...
    print("create product attribute line ids")
...
def create_attribute(option_data, r, odoo):
    ...
    for option in option_data:
        ...
        # )
        attribute_id = None
        attribute_id = odoo.search('product.attribute', [('name', '=', option['name']),('create_variant','=', 'always')], lang=lang)
        print("search odoo attribute " + str(attribute_id) + " "+ option['name'])
        if attribute_id:
            ...
            for value in option['values']:
                ...
                # )
                attribute_value_id = odoo.search('product.attribute.value', [('name', '=', value),('attribute_id','=',attribute_id)], lang=lang)
                print("search attribute value id "+ str(attribute_value_id))
                print(f'attribute_values:{option["name"]}:{value}')
                print(attribute_value_id[0])
                if attribute_value_id:
                    ...
                    #     }]
                    # )
                    attribute_value_id = odoo.create('product.attribute.value', {
                        'name': value,
                        'attribute_id': attribute_id,
                    }, lang=lang)
                    print(attribute_value_id)
...
def update_product_variants(product_id, option_data, attribute_line_ids, variant_data, images, r, odoo):
    ...
    template_attribute_line_ids = odoo.search_read('product.product', search_criteria, fields, lang=None)
    #print(template_attribute_line_ids)
    #exit()
    ...
    for template_attribute_line in template_attribute_line_ids:
        ...
        for attribute_line in attribute_line_ids:
            ...
            attribute_value = odoo.search_read('product.template.attribute.value', [['id', '=', attribute_line]], ['name'], lang=lang)
        ...
        if default_code:
            ...
            print(default_code + " " + str(template_attribute_line['id']) + lang)
...
if __name__ == '__main__':
    ...
    product = get_product("10143471730961", False)
    ...
    for product in products:
        ...
        print(option_data)
        create_attribute_result = create_attribute(option_data, r, odoo)
        print(create_attribute_result)
        ...
        product_id = create_product(product_data, variant_data, option_data, r, odoo)
        print(product_id)
        #exit()

        # update the product variants
        update_product_variants(product_id, option_data, attribute_line_ids,  variant_data, images, r, odoo) 
